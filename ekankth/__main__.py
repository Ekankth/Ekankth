# Copyright https://github.com/TeamKillerX/ekankth
# Create file by @xtsea
...
from telegram import __version__ as desah
...
from ekankth.modules import ALL_MODULES
from ekankth.modules.helper_funcs.chat_status import is_user_admin
from ekankth.modules.helper_funcs.decorators import rencmd, rencallback, renmsg
from ekankth.modules.helper_funcs.misc import paginate_modules
from ekankth.modules.language import gs
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("ekankth.modules." + module_name)
...
@rencallback(pattern=r"start_back")
@rencmd(command="start", pass_args=True)
            ...
            if args[0].lower() == "help":
                ...
                help_buttons.append(
                    [
                        ...
                        InlineKeyboardButton(
                            text="Support", url=f"https://t.me/pantekys"
                        ),
                    ]
                )
                send_help(
...
@rencmd(command="donate")
def donate(update: Update, _: CallbackContext):
    update.effective_message.reply_text("I'm free donate>_<")


@renmsg(Filters.status_update.migrate)
...
def main():
    ...
    if WEBHOOK:
        ...
        log.info(
            f"Ekankth started, Using long polling. | BOT: [@{dispatcher.bot.username}]"
        )
        EkankthINIT.bot_id = dispatcher.bot.id
        EkankthINIT.bot_username = dispatcher.bot.username
        EkankthINIT.bot_name = dispatcher.bot.first_name
...
if __name__ == "__main__":
    log.info("[Ekankth] Successfully loaded modules: " + str(ALL_MODULES))
